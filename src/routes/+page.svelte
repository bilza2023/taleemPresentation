<script>
    // No additional script needed
  </script>
  
  <div class="bg-gray-800 text-white min-h-screen p-8">
    <h1 class="text-3xl font-bold mb-8 border-b-2 border-white pb-2">Taleem Presentation</h1>
  
    <ul class="space-y-6 text-lg font-semibold">

      <li><a href="/player" class="block border-b border-white pb-2 text-green-600">Player</a></li>
      
      <li><a href="/editor" class="block border-b border-white pb-2 text-green-400">Editor:&nbsp; <span class="text-gray-300 text-xs">"isBlob = true" and audioData is a blob. </span></a>
      </li>


    </ul>
 

</div><!--page ends-->
    