<script>
  ...
  // Reactive assignment: Automatically updates `pulse` when `presentationObj.pulse()` changes
  $: pulse = presentationObj.pulse();
...
<div class='toolbar-wrapper' in:fade={{ delay: 300 }} out:fade={{ delay: 300 }}>
  <div class='toolbar-content flex items-center space-x-2'>
      ...
      <div class="w-2/12 p-1 rounded-sm bg-gray-700 text-yellow-500 text-xs text-center">
        {pulse.toFixed(0)} / {presentationObj.slides[presentationObj.slides.length - 1].endTime} sec
      </div>
